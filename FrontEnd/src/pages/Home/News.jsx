...
function News(){
    return(
        <>
        <div className="py-16">
            <h2 className="text-3xl font-sembold">News</h2>
            <Swiper
        slidesPerView={3}
        spaceBetween={30}
        navigation={true}
            ...
            news.map((val,i)=>{
                return  <SwiperSlide key={i}>
                    <div className="flex flex-col sm:flex-row sm:justify-between items-center  ">
                        <div className="py-4">
                            <Link to="/"><h3 className="text-lg font-medium hover:text-blue-500">{val.title}</h3></Link>
                        </div>
                        <div className="w-12 h-[4px] bg-primary mb-5"></div>
                        <p className="text-sm">{val.description}</p>
                        <div className="flex-shrink-0">
                            <img  src={val.image} alt="" className="w-full object-cover " />
                        </div>
                    </div>
                </SwiperSlide>
